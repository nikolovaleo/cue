...
contextBridge.exposeInMainWorld('cue', {
  ...
  setProtectedMouseRegions: (regions) => ipcRenderer.send('mouse:regions', regions),
  ...
  on: (channel, cb) => {
    const allowed = ['capture:state', 'llm:start', 'llm:token', 'llm:done', 'llm:error', 'llm:queued', 'llm:needs-confirmation', 'status', 'transcript', 'stt:interim', 'stt:final', 'stt:status', 'vad:state', 'applink:consent-request', 'hide:toggle', 'whisper:download-progress', 'whisper:models-changed'];
